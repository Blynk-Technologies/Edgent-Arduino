...
class NetMgrEsp8266WiFi {
    ...
    public:
        ApEntry(int8_t id)
            : id(id), rssi(0), channel(0), hidden(false) {
    ...
    public:
        ...
        size_t save(uint8_t* buff, size_t size) const {
            ...
            if (ssid.length()) {
                tlv.writeUInt8(FIELD_SSID);
                tlv.write(ssid);
            }
            if (psk.length()) {
                tlv.writeUInt8(FIELD_PSK);
                tlv.write(psk);
            }
            if (hasBSSID()) {
                tlv.writeUInt8(FIELD_BSSID);
                tlv.write(bssid, sizeof(bssid));
            }
            if (channel) {
                tlv.writeUInt8(FIELD_CHANNEL);
                tlv.writeUInt8(channel);
            }
            if (hidden) { tlv.writeUInt8(FIELD_HIDDEN); }

            if (localIP) {
                tlv.writeUInt8(FIELD_LOCAL_IP);
                tlv.write(localIP);
            }
            if (gateway) {
                tlv.writeUInt8(FIELD_GATEWAY);
                tlv.write(gateway);
            }
            if (subnet) {
                tlv.writeUInt8(FIELD_MASK);
                tlv.write(subnet);
            }
            if (dns1) {
                tlv.writeUInt8(FIELD_DNS);
                tlv.write(dns1);
            }
            if (dns2) {
                tlv.writeUInt8(FIELD_DNS);
...
public:
    ...
    bool scanGetResult(int i, String& ssid, String& sec,
        ...
        if (!WiFi.getNetworkInfo(i, ssid, res.sec,
                                 rssi, res.bssid,
                                 chan, res.hidden) ||
            !res.bssid) {
            return false;
...
public:
    static const char* wifiSecToStr(int sec) {
        switch (sec) {
...
private:
    ...
    void processScanResults(const int scanResult) {
        ...
        for (int i = 0; i < scanResult; ++i) {
            ...
            bool res = WiFi.getNetworkInfo(i, ssid_scan, sec_scan, rssi_scan, bssid_scan, chan_scan, hid_scan);
            if (!res || !bssid_scan) {
                continue;
            }
